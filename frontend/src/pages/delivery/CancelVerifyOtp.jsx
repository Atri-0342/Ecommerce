...
const CancelVerifyOtp = () => {
  ...
  const themeColor = "#10b981"; // Consistent Emerald Green
  const OTP_LENGTH = 4;

  const handleVerifyCancel = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      setError(err.response?.data?.message || "Verification failed. Check OTP and try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ 
      ...
      <Card className="border-0 shadow-lg" style={{ width: '90%', maxWidth: '400px', borderRadius: '20px' }}>
        ...
        <div className="text-center p-4">
          ...
          <p className="text-muted small">Verification required to cancel this delivery</p>
        </div>
        ...
        <Card.Body className="px-4 pb-4 pt-0">
          ...
          <Form onSubmit={handleVerifyCancel}>
            {/* OTP Input Field */}
            <div className="mb-4">
              <Form.Control
                type="text"
            ...
            <Form.Group className="mb-4">
              <Form.Label className="small fw-bold text-muted ms-1">Reason for Cancel</Form.Label>
              <Form.Select 
                value={cancelReason} 
                onChange={(e) => setCancelReason(e.target.value)}
                className="border-2 shadow-none py-2"
                style={{ borderRadius: '10px', borderColor: '#d1fae5' }}
                disabled={loading}
                required
              >
                ...
                <option value="Vehicle/Technical issue">Vehicle/Technical issue</option>
              </Form.Select>
            </Form.Group>
            ...
            <div className="d-grid gap-2">
              <Button 
                type="submit" 
                disabled={loading || otp.length !== OTP_LENGTH || !cancelReason}
              ...
              >
                {loading ? (
                  ...
                  <span className="d-flex align-items-center justify-content-center">
                    <WarningCircle size={20} className="me-2" /> Complete Cancel
                  </span>
                )}
              </Button>
...
export default CancelVerifyOtp;